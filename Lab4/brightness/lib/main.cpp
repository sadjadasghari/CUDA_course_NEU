...
extern unsigned char *brightness(unsigned char *intensity, 
		unsigned int height, 
		unsigned int width);

extern unsigned char *brightnessWarmup(unsigned char *intensity, 
		unsigned int height, 
		unsigned int width);
		
extern void modBrightness (char value);
...
int main(int argc, char* argv[]) {
	...
	#if defined(WARMUP)
		cpu.result = brightnessWarmup(cpu.intensity, 
			   cpu.height, 
			   cpu.width);
	#endif
